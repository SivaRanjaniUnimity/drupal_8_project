(function ($) {
  Drupal.behaviors.hoya_job_openings = {
      ...
      $(context).find("input[name*='job_location']").once().click(function () {
        jQuery("#result-container").addClass("no-result");
      jQuery("#result-container").html("<div class='gif-loader'></div>");   
      // $("body",context).once().on("click", "input[name*='job_location111']", function(){
  ...
  function callAnychart(locationData){
    anychart.licenseKey("hoya.com-48b79d66-90291622");
    ...
    // once('#map-loc-container', 'html', context).forEach( function (element) {
      ...
      mapSeries.size(15).geoIdField('code_hasc');
        ...
        var image_link = '/modules/custom/hoya_job_openings/images/noun-location.png';
      ...
      mapSeries
        ...
        .format(function () {
          return (
            '<div style="width:80px;height:55px;background-color: #FFFFFF; display: flex; justify-content:center; align-items: center;"><div style="position: absolute; "><div style="width:26px; height:26px; border-radius:13px; color:#fff; line-height:26px; text-align:center; background:#3479C6; margin: 0 auto;"></div><div style="padding-top:3px;color: #0057B8;text-align:center;">' +
            this.getData('name') +
            '</div></div></div>'
          );
        });
    ...
    jQuery.ajax(
      {
        ...
        dataType: "json",
        async: false,
        data : {"filter_id":joblocation_array},
        success: function (result) {
          console.log(result);
        if(info == "info1"){
